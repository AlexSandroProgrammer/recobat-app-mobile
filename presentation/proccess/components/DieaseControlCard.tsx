...
export const DieaseControlCard = ({
  title,
  description,
  weed_control_proccesses: weedControl,
  disease_control_proccesses: dieaseControlProccess,
}: CardSubProccessProps) => {
  const firstWeedControl = weedControl ? weedControl[0] : null;

  const firstDieaseControl = dieaseControlProccess
    ? dieaseControlProccess[0]
    : null;
  return (
    <View
    ...
    >
      ...
      <View className="flex flex-row items-end justify-center mt-3">
        {firstWeedControl?.status_weed_control === "finish" &&
        firstDieaseControl?.status_diesease_control === "enabled" ? (
          <TouchableOpacity
            onPress={() => router.push("/")}
